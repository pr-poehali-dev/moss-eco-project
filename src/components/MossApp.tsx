...
// ─── Data ─────────────────────────────────────────────────────────────────────
const HERO_IMG = "https://cdn.poehali.dev/projects/ad3c7dd6-d380-475d-bcf2-32abf5134c2e/files/1cd145cc-017f-453f-97d0-d58cb2e27e7d.jpg";
const MOSS_COLLECTION_IMG = "https://cdn.poehali.dev/projects/ad3c7dd6-d380-475d-bcf2-32abf5134c2e/files/16f7125f-6f00-45fe-9ed6-07255fccd602.jpg";
const LICHEN_IMG = "https://cdn.poehali.dev/projects/ad3c7dd6-d380-475d-bcf2-32abf5134c2e/files/18d542e3-fe1c-417a-a04d-06cca84624dd.jpg";

const PRODUCTS: Product[] = [
  { id: 1, category: "Ягель", name: "Премиум мох", nameEn: "Premium Moss", description: "Только крупный мох, плотные шапочки, отсутствие отходов. 25 оттенков.", descriptionEn: "Large moss only, dense caps, zero waste. 25 shades.", price: 2400, badge: "Хит", colors: 25, image: LICHEN_IMG },
  { id: 2, category: "Ягель", name: "Стандартный ягель", nameEn: "Standard Lichen", description: "Классический ягель для декора и флористики. 25 оттенков.", descriptionEn: "Classic reindeer lichen for decor and floristry. 25 shades.", price: 1600, colors: 25, image: LICHEN_IMG },
  { id: 3, category: "Ягель", name: "Флористический ягель", nameEn: "Floral Lichen", description: "Мелкий мох, короткие и маленькие пучки. Идеален для флористических композиций.", descriptionEn: "Fine moss, short and small bunches. Perfect for floral arrangements.", price: 1800, colors: 25, image: LICHEN_IMG },
  { id: 4, category: "Королевский мох", name: "Королевский мох", nameEn: "Royal Moss", description: "8 оттенков зелёного. Роскошная текстура для премиальных интерьеров.", descriptionEn: "8 shades of green. Luxurious texture for premium interiors.", price: 3200, badge: "Премиум", colors: 8, image: MOSS_COLLECTION_IMG },
  { id: 5, category: "Плоский мох", name: "Плоский мох", nameEn: "Flat Moss", description: "2 цвета. Ровная бархатистая поверхность, идеальна для панелей.", descriptionEn: "2 colors. Smooth velvety surface, ideal for panels.", price: 1900, colors: 2, image: MOSS_COLLECTION_IMG },
  { id: 6, category: "Мшанка", name: "Мох мшанка", nameEn: "Irish Moss", description: "2 цвета. Нежная мелкая текстура, создаёт эффект живого газона.", descriptionEn: "2 colors. Delicate fine texture, creates living lawn effect.", price: 2100, colors: 2, image: MOSS_COLLECTION_IMG },
  { id: 7, category: "Панели", name: "Готовые панели из мха", nameEn: "Ready Moss Panels", description: "Готовые интерьерные панели на основе. Размеры на заказ.", descriptionEn: "Ready interior panels on base. Custom sizes available.", price: 8500, badge: "Популярное", image: HERO_IMG },
  { id: 8, category: "Проекты", name: "Индивидуальный проект", nameEn: "Custom Project", description: "Уникальные инсталляции, логотипы, панно. Индивидуальный выкрас под ваш интерьер.", descriptionEn: "Unique installations, logos, murals. Custom dyeing for your interior.", price: 0, badge: "На заказ", image: HERO_IMG },
];

const REVIEWS = [
  { id: 1, name: "Анастасия К.", role: "Дизайнер интерьеров", text: "Заказываю ягель уже третий раз. Качество стабильно высокое, цвета точно совпадают с каталогом. Отгрузка реально за 1 день!", textEn: "I order lichen for the third time. Quality is consistently high, colors match the catalog exactly. Shipment really within 1 day!", rating: 5 },
  { id: 2, name: "Михаил Р.", role: "Владелец ресторана", text: "Сделали мох-панно для нашего ресторана. Работа выполнена безупречно, клиенты постоянно фотографируются у стены.", textEn: "Made a moss mural for our restaurant. Flawless work, customers constantly take photos by the wall.", rating: 5 },
  { id: 3, name: "Елена В.", role: "Флорист", text: "Флористический ягель — находка! Идеальный размер бочков, никаких отходов. Уже рекомендую коллегам.", textEn: "Floral lichen is a gem! Perfect bunch size, zero waste. Already recommending to colleagues.", rating: 5 },
];
      ...
      <nav className="moss-nav">
        <div className="moss-container moss-nav__inner">
          ...
          <div className="moss-nav__actions">
            <button className="moss-lang" onClick={() => setLang(lang === "ru" ? "en" : "ru")}>
              {lang === "ru" ? "EN" : "RU"}
            </button>
            ...
            <button className="moss-burger" onClick={() => setMobileMenu(!mobileMenu)}>
        ...
        {mobileMenu && (
          <div className="moss-mobile-menu">
            {navItems.map((item) => (
              <button key={item.key} onClick={() => { setPage(item.key); setMobileMenu(false); }} className="moss-mobile-menu__item">
                {item.label}
              </button>
            ))}
            <button onClick={() => { setPage("cart"); setMobileMenu(false); }} className="moss-mobile-menu__item">
          ...
          <section className="moss-hero">
            ...
            <div className="moss-container moss-hero__content">
              ...
              <div className="moss-hero__btns animate-fade-in">
                ...
                <button className="moss-btn moss-btn--outline" onClick={() => {
          ...
          <section id="custom-section" className="moss-custom">
            <div className="moss-container moss-custom__inner">
              <div className="moss-custom__text">
                <span className="moss-tag moss-tag--dark">{lang === "ru" ? "Под ваш проект" : "Tailor-made"}</span>
                ...
                <button className="moss-btn moss-btn--primary" onClick={() => {
          ...
          <div className="moss-container">
            ...
            <div id="order-form-cat" style={{ paddingTop: "4rem" }}>
              ...
              {orderSent ? (
                ...
                <form className="moss-order-form" onSubmit={handleOrderSubmit}>
                  <input className="moss-input" placeholder={t.order.name} value={orderForm.name} onChange={(e) => setOrderForm({ ...orderForm, name: e.target.value })} required />
                  <input className="moss-input" placeholder={t.order.phone} value={orderForm.phone} onChange={(e) => setOrderForm({ ...orderForm, phone: e.target.value })} required />
                  <textarea className="moss-input moss-textarea" placeholder={t.order.message} value={orderForm.message} onChange={(e) => setOrderForm({ ...orderForm, message: e.target.value })} rows={4} />
                  <button type="submit" className="moss-btn moss-btn--primary moss-btn--full">{t.order.submit}</button>
                </form>
              )}
            </div>
          </div>
                  ...
                  {cart.map((item) => (
                    <div key={item.id} className="moss-cart-item">
                      ...
                      <div className="moss-cart-item__info">
                        <div className="moss-cart-item__name">{lang === "ru" ? item.name : item.nameEn}</div>
                        <div className="moss-cart-item__category">{item.category}</div>
                        <div className="moss-cart-item__price">
                          {item.price > 0 ? `${(item.price * item.qty).toLocaleString()} ₽` : t.catalog.custom}
                        </div>
                      </div>
          ...
          <div className="moss-container moss-account">
            ...
            {isLoggedIn ? (
              ...
              <div className="moss-account__auth">
                ...
                <form className="moss-order-form" onSubmit={(e) => { e.preventDefault(); setIsLoggedIn(true); }}>
          ...
          <div>
            ...
            <div className="moss-footer__links">
              {["Ягель", "Королевский мох", "Плоский мох", "Мшанка", "Панели"].map((cat) => (
                <button key={cat} className="moss-footer__link" onClick={() => { setPage("catalog"); setFilterCat(cat); }}>
                  {cat}
                </button>
              ))}
            </div>
